...
class Admin
{
    ...
    public function set_user($user_name,$email,$password,$first_name,$last_name,$phone_number,$address,$user_ion_id,$profile_image,$auth_key)
    {
    ...
    // update user data
    public function update_user($db_conn)
    {
        $user_name=$this->user_name;
        $email=$this->email;
        $password=$this->password;
        ...
        $query=$db_conn->prepare("UPDATE merchants SET user_name=?,email=?,password=?,first_name=?,last_name=?,phone_number=?,address=?,user_ion_id=?,profile_image=?,updated_at=? WHERE user_ion_id=?");
        $query->bind_param("sssssisssss", $user_name,$email,$password,$first_name,$last_name,$phone_number,$address,$user_ion_id,$profile_image,$updated_at,$user_ion_id);
        $query->execute();
        if ($query->affected_rows >0){
            return true;
        }
    ...
    public function charge_amount($merchant_ion_id,$remaining_cradit,$db_conn)
    {
        ...
        $user_credit->bind_param("dsss",$remaining_cradit,$updated_at,$merchant_ion_id);
        $user_credit->execute();
