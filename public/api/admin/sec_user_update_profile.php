...
if ($_SERVER['REQUEST_METHOD']=== "POST") {
    ...
    $data = json_decode(file_get_contents('php://input'));
        ...
        if (!empty($jwt_key)) {
            try{
                if (!empty($data->user_ion_id)) {
                    
                    $get_user=$user->find_user("secondary_user",'user_ion_id',$data->user_ion_id,$conn);
                    if(!empty($get_user)){
                        $varify=$varification->get_user_data($jwt_key);
                        if(!empty($varify['decoded_data'])){

                            // $merchant_id=$varify['decoded_data']->data->merchant_id;
