...
\Stripe\Stripe::setApiKey('sk_test_51HfMAJKav7v2b6Ix55oZIL4qloYEYvZ4O0neZqoALRQiqGtvM5N6xjgYu3BmO5VL38g7bYVn8USOtnHsEHAY3Fwy00zs1zXo4Z');
    ...
    if ($_SERVER['REQUEST_METHOD']=== "POST") {
        ...
        $data = json_decode(file_get_contents('php://input'));
                ...
                try
                    {  
                        ...
                        if(!empty($varify['decoded_data'])){
                            ...
                            if ($varify['decoded_data']->data->cradit_recharge=1 ) {
                                if ( $user_merchant_id===$data->merchant_id) {
                                    if (!empty($user_merchant_id)) {
                                      
                                        $get_user=$user->find_user("secondary_user","user_ion_id",$user_merchant_id,$conn); 
                                        print_r($get_user);
                                        exit;
                                        if(!empty($get_user)){
                                            ...
                                            $card_data=$mercharnt->find_cradit_card($data->card_number,$user_merchant_id,$conn);
                                            ...
                                            if ($card_data) {
                                                ...
                                                $mercharnt->add_card($user_merchant_id,$data->card_number,$data->exp_month,$data->exp_year,$data->cvc,$customer->id,$conn);
                                            }
                                            if ($charge_success) {

                                              // update cradit of user in database
                                              $mercharnt->update_cradit($user_merchant_id,$conn);
                                              ...
                                              $transaction->set_transaction($transaction_id,$customer_id,$user_merchant_id,$product,$amount,$currency,$status);
                                            ...
                                            $response->set_error_response(null,"Invalid user id", "404","you are not eligible");
                                ...
                                $response->set_error_response(null,"not allowed to Recharge", "404","you are not allowed to Recharge this account please use corrrect marchant id");
                                echo $response->error_respond_api();
                            }
                        }
                    }catch (Exception $ex) {
            ...
            $response->set_error_response(null,"Request not found", "404","Only Post request allowed");
            echo $response->error_respond_api();
        }
